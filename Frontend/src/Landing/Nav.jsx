...
export default function Nav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-6 px-4 pointer-events-none">
      <div className="pointer-events-auto backdrop-blur-2xl bg-black/70 rounded-full border border-white/[0.06] px-5 py-2.5 flex items-center gap-8 w-max shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
        ...
        <div className="hidden md:flex items-center gap-6">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-sm text-cool-gray/80 hover:text-glacier-white transition-colors duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
            >
              {link.label}
            </a>
          ))}
        ...
        <a
                href="/auth"
          className="hidden md:inline-flex items-center gap-2 rounded-full bg-crisis-red px-4 py-1.5 text-sm font-medium text-white transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:scale-105 active:scale-[0.98]"
        >
          Access Dashboard
        </a>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="pointer-events-auto absolute top-20 left-4 right-4 backdrop-blur-3xl bg-black/80 rounded-2xl border border-white/[0.06] p-6 shadow-[0_16px_48px_rgba(0,0,0,0.5)]"
          >

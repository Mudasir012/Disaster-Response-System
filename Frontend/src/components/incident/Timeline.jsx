  ...
  return (
    <div className="bg-surface border border-white/[0.08] rounded-xl p-5">
      <h3 className="text-sm font-semibold text-glacier-white mb-4 flex items-center gap-2">
        <Clock size={16} className="text-cool-gray" />
        Source Timeline
      </h3>
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  {src.processed && <span className="text-[10px] text-status-teal">✓ AI processed</span>}
                </div>
                <p className="text-sm text-cool-gray mt-1 line-clamp-2">{src.raw_text}</p>
              </div>
              <span className="text-[11px] text-cool-gray/60 shrink-0">{formatTime(src.fetched_at)}</span>
            </div>
          </div>
        ))}
      </div>

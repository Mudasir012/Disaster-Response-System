    ...
    <div className="min-h-screen bg-deep-slate">
      ...
      <div className="pt-14">
        <div className="max-w-3xl mx-auto px-4 py-12">
          ...
          <div className="bg-amber/10 border border-amber/30 rounded-xl p-4 mb-10 flex items-start gap-3">
          ...
          <div className="mb-12">
            ...
            <div className="space-y-0 relative">
              {[
                ...
                <div key={item.step} className="flex gap-4 pb-8 last:pb-0 relative">
                  <div className="shrink-0 w-12 h-12 rounded-full bg-signal-blue/20 border-2 border-signal-blue flex items-center justify-center">
                    <item.icon size={20} className="text-signal-blue" />
                  </div>
                  <div>
                    <h3 className="text-base font-semibold text-glacier-white">{item.title}</h3>
                    <p className="text-sm text-cool-gray mt-1">{item.body}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#1a1040] border border-ai-purple/30 rounded-xl p-6 mb-10 border-l-4 border-l-ai-purple">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-[10px] font-semibold text-ai-purple bg-ai-purple/20 px-2 py-0.5 rounded">ABOUT OUR AI</span>
            </div>
            ...
            <ul className="text-sm text-cool-gray space-y-1 list-disc list-inside">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {dataSources.map((ds) => (
                <a key={ds.name} href={ds.url} target="_blank" rel="noopener noreferrer"
                  className="bg-surface border border-white/[0.08] rounded-xl p-4 hover:border-white/20 transition-all group">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold text-glacier-white">{ds.name}</span>
                    <ExternalLink size={14} className="text-cool-gray group-hover:text-glacier-white transition-colors" />
                  </div>
                  <p className="text-xs text-cool-gray">{ds.desc}</p>
                </a>
              ))}
            </div>
          </div>
          ...
          {statsSummary && (
            <div className="text-center mb-8">
              <div className="text-sm text-cool-gray">
                <span className="font-semibold text-glacier-white">{statsSummary.total?.toLocaleString() || '—'}</span> incidents tracked to date
              </div>
            </div>
          )}
          ...
          <div className="text-center mb-8">
            <div
              ...
              className="inline-block text-sm text-cool-gray hover:text-glacier-white cursor-pointer transition-colors"
            >
              {expanded ? 'Hide' : 'Show'} tech stack details
            </div>
            {expanded && (
              <div className="mt-4 bg-surface border border-white/[0.08] rounded-xl p-6 text-left">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {[
                    ...
                    <div key={label}>
                      <span className="text-cool-gray">{label}:</span>
                      <span className="text-glacier-white ml-1">{value}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="text-center pb-8">
            <Link to="/map"
              className="inline-flex items-center gap-2 bg-crisis-red hover:bg-crisis-red/90 text-white font-semibold px-6 py-3 rounded-xl transition-all hover:scale-[1.02]">
              View Live Map
              <ArrowRight size={18} />
            </Link>
          </div>

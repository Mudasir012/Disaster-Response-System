...
export default function MapDashboard() {
  ...
  return (
    <main className="h-screen flex flex-col bg-deep-slate">
      ...
      <div className="flex-1 relative overflow-hidden pt-14">
        ...
        {!sidebarOpen && (
          <button
            ...
            className="fixed left-3 top-16 z-30 p-2.5 bg-surface/80 backdrop-blur-sm border border-white/10 rounded-lg text-cool-gray hover:text-glacier-white transition-all hover:bg-surface"
          >
            <Menu size={18} />
          </button>
        )}
        ...
        <button
          ...
          className="md:hidden fixed left-3 top-16 z-30 p-2.5 bg-surface/80 backdrop-blur-sm border border-white/10 rounded-lg text-cool-gray"
        >
          <Menu size={18} />
        </button>

        {(mobileSidebar || mobileClosing) && (
          <div className={`fixed inset-0 z-30 md:hidden ${
            mobileClosing ? 'animate-fade-out' : 'animate-fade-in'
          }`}>
            <button
              className="absolute inset-0 bg-black/60 backdrop-blur-sm"
              onClick={() => { setMobileClosing(true); setTimeout(() => { setMobileSidebar(false); setMobileClosing(false) }, 200) }}
              aria-label="Close sidebar"
            />
            <div className={`absolute left-0 top-14 bottom-0 w-[85vw] max-w-[300px] bg-deep-slate/95 backdrop-blur-xl border-r border-white/[0.08] overflow-y-auto p-4 ${

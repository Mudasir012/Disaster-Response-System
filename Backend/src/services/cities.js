...
export function searchCities(query, limit = 10) {
  ...
  for (const city of index) {
    if (city.nameLower.startsWith(q)) {
      results.push({ name: city.name, country: city.country, population: city.population, lat: city.lat, lng: city.lng })
...
const countryNames = {
  ...
  CO: 'Colombia', KM: 'Comoros', CG: 'Congo', CD: 'Congo DR', CR: 'Costa Rica', CI: "Côte d'Ivoire",
  ...
  KI: 'Kiribati', KP: 'North Korea', KR: 'South Korea', KW: 'Kuwait', KG: 'Kyrgyzstan', LA: 'Laos',
  ...
  VA: 'Vatican City', VE: 'Venezuela', VN: 'Vietnam', YE: 'Yemen', ZM: 'Zambia', ZW: 'Zimbabwe', XK: 'Kosovo',
}
...
export const countries = Object.entries(countryNames).map(([code, name]) => ({
  code,
  name,
